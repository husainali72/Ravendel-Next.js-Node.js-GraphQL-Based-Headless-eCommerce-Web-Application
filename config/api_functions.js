...
const CREATE_FUNC = async (token, name, modal, data, method, path,args) => {
  if (!token) {
    return MESSAGE_RESPONSE("TOKEN_REQ", name, false);
  }
  checkToken(token);
  try {
    ...
    if (args.feature_image) {
      let imgObject = "";
   //   if (args.feature_image) {
     //   console.log('1111',data.feature_image);
        imgObject = await imageUpload(data.feature_image.file, path);
...
const UPDATE_FUNC = async (
  ...
  try {
    ...
    if (response) {
      if (args.updatedImage.file) {
        console.log('1111',args.updatedImage);
...
//     } else {
//       return {
//         message: MESSAGE_RESPONSE("NOT_EXIST", name, false),
//         data: response,
//       };
//     }
//   } catch (error) {
...
//     }
//     if (data.code) {
//       const response = await modal.findOne({ code: data.code });
//       if (response) {
//         return MESSAGE_RESPONSE("DUPLICATE", "couponCode", false);
//       }
//     }
//     if (data) {
//       await modal.insertMany(data);
//     }
//     const response = new modal(data);
//     await response.save();
//     return MESSAGE_RESPONSE("AddSuccess", name, true);
