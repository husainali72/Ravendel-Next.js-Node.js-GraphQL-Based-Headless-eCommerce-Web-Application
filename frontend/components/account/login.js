...
import { createCart, updateCart } from "../../redux/actions/cartAction";
import Link from "next/link";
import Loading from "../loading";
import { capitalize } from "lodash";
import PasswordField from "../passwordField";
import { LoginValidation } from "../../utills/Validation";
...
const LogIn = () => {
  ...
  const doLogin = async (e) => {
    const errors = LoginValidation(loginUser);
    console.log(errors)
    setError((prevErrors) =>console.log(prevErrors,'prevErrors'));
    setError( {  ...errors });
    e.preventDefault();
  ...
  return (
    <div className="login-box ">
      ...
      <form onSubmit={doLogin}>
        ...
        >
          <span>Login {loading && <Loading />}</span>
        </button>
        {loginSuccess ? <h4>Login success full</h4> : null}
        {error ? <p style={{ color: "red" }}>{capitalize(error?.passsword)}</p> : null}
      </form>
    </div>
  );
};
