...
const GalleryImagesComponents = (props) => {
  ...
  const addToCartProduct = async (product) => {
    if (
      singleproducts.attribute_master.length > 0 &&
      selectedAttrs.length !== singleproducts.attribute_master.length
    ) {
      seterror(true);
    } else {
      seterror(false);
      ...
      if (session.status === "authenticated") {
        ...
        query(GET_USER_CART, id, token).then((res) => {
          ...
          inCartProducts?.map((inCartProduct) => {
            if (
              product._id === inCartProduct.productId &&
              comboData.some(
              ...
              let Cart = inCartProducts.map((item) => {
                if (
                  comboData.some((variant) => variant.id === item.variantId)
                ) {
                  return {
          ...
          if (!productInCart) {
            if (comboData.length > 0) {
              ...
              let variables = {
                total:
                  comboData[0]?.pricing?.sellprice * quantity ||
                  comboData[0]?.pricing?.price * quantity ||
                  product?.pricing?.sellprice * quantity ||
